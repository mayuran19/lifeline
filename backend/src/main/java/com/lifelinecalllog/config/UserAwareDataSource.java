...
/**
 * DataSource wrapper that sets app.current_user (as UUID) once per connection acquisition. Looks up
 * the authenticated user's UUID from app_user by username so audit triggers record the user's ID
...
public class UserAwareDataSource extends DelegatingDataSource {
  ...
  private void applyCurrentUser(Connection connection) throws SQLException {
    ...
    try (PreparedStatement stmt =
        connection.prepareStatement("SELECT set_config('app.current_user', ?, false)")) {
  ...
  private String lookupUserId(Connection connection, String username) throws SQLException {
    try (PreparedStatement stmt =
        connection.prepareStatement("SELECT id FROM app_user WHERE username = ?")) {
