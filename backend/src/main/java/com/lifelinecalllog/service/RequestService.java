...
public class RequestService {
    ...
    public List<RequestResponse> findAll(RequestStatus status, UUID clinicId) {
    ...
    public RequestResponse create(RequestCreateRequest req) {
        ...
        record.setStartTime(req.startTime());
        record.setEndTime(req.endTime());
        record.store();
    ...
    public RequestResponse update(UUID id, RequestUpdateRequest req) {
        var record = dsl.selectFrom(REQUEST)
                .where(REQUEST.ID.eq(id))
                .fetchOptional()
                .orElseThrow(() -> new IllegalArgumentException("Request not found: " + id));
    ...
    public RequestResponse removePatient(UUID requestId, UUID requestPatientId) {
        int deleted = dsl.deleteFrom(REQUEST_PATIENT)
                .where(REQUEST_PATIENT.ID.eq(requestPatientId)
                        .and(REQUEST_PATIENT.REQUEST_ID.eq(requestId)))
                .execute();
        if (deleted == 0) throw new IllegalArgumentException("Request patient not found: " + requestPatientId);
        return findById(requestId);
    }
    ...
    private org.jooq.SelectOnConditionStep<Record> buildSelect() {
        ...
        return dsl.select(fields)
                .from(REQUEST)
                .join(CLINIC).on(REQUEST.CLINIC_ID.eq(CLINIC.ID))
                .leftJoin(DOCTOR).on(REQUEST.DOCTOR_ID.eq(DOCTOR.ID));
    }
    ...
    private List<RequestResponse.RequestPatientResponse> fetchPatients(UUID requestId) {
        return dsl.select(
                ...
                .join(PATIENT).on(REQUEST_PATIENT.PATIENT_ID.eq(PATIENT.ID))
                .where(REQUEST_PATIENT.REQUEST_ID.eq(requestId))
                .orderBy(PATIENT.LAST_NAME, PATIENT.FIRST_NAME)
                .fetch(r -> new RequestResponse.RequestPatientResponse(
    ...
    private RequestResponse toResponse(Record r, List<RequestResponse.RequestPatientResponse> patients) {
